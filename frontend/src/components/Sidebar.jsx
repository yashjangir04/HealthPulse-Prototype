...
const Sidebar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <aside 
      className={`fixed left-0 top-24 h-[calc(100vh-6rem)] bg-white/80 backdrop-blur-md border-r border-gray-100 transition-all duration-500 ease-in-out z-50 flex flex-col justify-between py-6 ${isCollapsed ? 'w-16' : 'w-64'}`}
    >
      {/* Toggle Button */}
      <button 
      ...
      >
        {isCollapsed ? <ChevronRight size={16} className="text-blue-600" /> : <ChevronLeft size={18} className="text-blue-600" />}
      </button>
